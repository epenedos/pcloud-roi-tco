...
| 2026-08-04 | PVWA (Privileged Web Access) added to the t-shirt inventory matrix per product owner: 4/8/160 (Small, ×1), 8/16/160 (Mid-Range, ×2), 16/32/160 (Large, ×3), 32/64/160 (Very Large, ×4), Windows. Node counts follow the web-tier scaling pattern (not specified in the request). The reference workbook predates PVWA, so its inventory is frozen as `workbook_inventory` in the defaults registry — golden parity (€515,593) and the workbook demo keep using it, while new analyses at Mid-Range defaults now show €530,437 (+ PVWA €4,948/yr × 3). |
